...
def plot_pred_vs_true(
    ...
    ax_main.errorbar(
        y_true_1d, y_pred_1d, yerr=yerr, fmt="o", color="#539ecd", alpha=0.5
    )

    lim_min = float(min(y_true_1d.min(), y_pred_1d.min()))
    lim_max = float(max(y_true_1d.max(), y_pred_1d.max()))
    ax_main.plot(
        [lim_min, lim_max], [lim_min, lim_max], "r--", linewidth=1, label="y = x"
    )
...
def plot_confusion_matrix(y_true, y_pred, classes, title, savepath=None):
    ...
    if savepath is not None:
        os.makedirs(savepath, exist_ok=True)
        plt.savefig(
            os.path.join(savepath, f"{title}_confusion.png"), bbox_inches="tight"
        )
    plt.close(fig)
...
def plot_reliability_curve(y_true, y_proba, n_bins=10, title="", savepath=None):
    ...
    if savepath is not None:
        os.makedirs(savepath, exist_ok=True)
        plt.savefig(
            os.path.join(savepath, f"{title}_calibration.png"), bbox_inches="tight"
        )
    plt.close(fig)
